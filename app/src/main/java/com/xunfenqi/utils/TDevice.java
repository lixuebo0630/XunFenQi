...
public class TDevice {
	...
	public static int getDefaultLoadFactor() {
		if (_loadFactor == null) {
			Integer integer = Integer
					.valueOf(0xf & MyApplication.getInstance().getResources()
	...
	public static boolean hasBigScreen() {
		...
		if (_hasBigScreen == null) {
			boolean flag1;
			if ((0xf & MyApplication.getInstance().getResources()
	...
	public static final boolean hasCamera() {
		if (_hasCamera == null) {
			PackageManager pckMgr = MyApplication.getInstance()
					.getPackageManager();
			boolean flag = pckMgr
	...
	public static boolean hasInternet() {
		boolean flag;
		if (((ConnectivityManager) MyApplication.getInstance()
	...
	public static boolean isPackageExist(String pckName) {
		try {
			PackageInfo pckInfo = MyApplication.getInstance()
					.getPackageManager().getPackageInfo(pckName, 0);
			if (pckInfo != null)
				return true;
		} catch (NameNotFoundException e) {
			AbLogUtil.e(MyApplication.getInstance(), e.getMessage());
		}
		return false;
	}
	...
	public static String getCurCountryLan() {
		return MyApplication.getInstance().getResources().getConfiguration().locale
				.getLanguage()
				+ "-"
	...
	public static boolean isZhCN() {
		String lang = MyApplication.getInstance().getResources()
				.getConfiguration().locale.getCountry();
	...
	public static void gotoMarket(Context context, String pck) {
		if (!isHaveMarket(context)) {
			AbToastUtil.showToast(context, "你手机中没有安装应用市场！");
			return;
		}
		Intent intent = new Intent();
		intent.setAction(Intent.ACTION_VIEW);
	...
	public static String getIMEI() {
		TelephonyManager tel = (TelephonyManager) MyApplication.getInstance()
	...
	public static void openApp(Context context, String packageName) {
		Intent mainIntent = MyApplication.getInstance().getPackageManager()
				.getLaunchIntentForPackage(packageName);
		if (mainIntent == null) {
			mainIntent = new Intent(packageName);
		} else {
			AbLogUtil.d(context, "Action:" + mainIntent.getAction());
		}
	...
	public static void copyTextToBoard(String string) {
		if (TextUtils.isEmpty(string))
			return;
		ClipboardManager clip = (ClipboardManager) MyApplication
	...
	public static int getStatuBarHeight() {
		...
		try {
			...
			field = c.getField("status_bar_height");
			x = Integer.parseInt(field.get(obj).toString());
			sbar = MyApplication.getInstance().getResources()
					.getDimensionPixelSize(x);
