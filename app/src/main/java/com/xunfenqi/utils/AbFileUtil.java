...
/**
 * 
 * @ClassName: AbFileUtil
 * @Description: 文件操作类.
 * @author Xuebo Li
 * @date 2015-8-14 下午3:01:58
 * 
 */
public class AbFileUtil {
	...
	public static Bitmap getBitmapFromSD(File file, int type, int desiredWidth,
		...
		try {
			...
			if (type == AbImageUtil.CUTIMG) {
				bitmap = AbImageUtil.getCutBitmap(file, desiredWidth,
						desiredHeight);
			} else if (type == AbImageUtil.SCALEIMG) {
				bitmap = AbImageUtil.getScaleBitmap(file, desiredWidth,
						desiredHeight);
			} else {
	...
	public static String getCacheFileNameFromUrl(String url,
		...
		try {
			...
			name = Md5Util.md5Diagest(url) + suffix;
		} catch (Exception e) {
			e.printStackTrace();
		}
		return name;
	...
	public static void initFileDir(Context context) {
		...
		String downloadRootPath = File.separator
				+ AbAppConfig.DOWNLOAD_ROOT_DIR + File.separator
		...
		// 默认缓存目录.
		String cacheDownloadPath = downloadRootPath + AbAppConfig.CACHE_DIR
				+ File.separator;

		// 默认DB目录.
		String dbDownloadPath = downloadRootPath + AbAppConfig.DB_DIR
				+ File.separator;
