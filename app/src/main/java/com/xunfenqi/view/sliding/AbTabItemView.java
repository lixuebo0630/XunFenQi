...
/**
 * 
 * @ClassName: AbTabItemView
 * @Description: 表示一个TAB
 * @author Xuebo Li
 * @date 2015-8-17 下午2:05:59
 * 
 */
public class AbTabItemView extends LinearLayout {
	...
	/** 图片. */
	private Drawable mLeftDrawable, mTopDrawable, mRightDrawable,
			mBottomDrawable;
	...
	public void setTabCompoundDrawables(Drawable left, Drawable top,
		...
		if (mLeftDrawable != null) {
			mLeftDrawable.setBounds(leftBounds, topBounds, rightBounds,
					bottomBounds);
		}
		if (mTopDrawable != null) {
			mTopDrawable.setBounds(leftBounds, topBounds, rightBounds,
					bottomBounds);
		}
		if (mRightDrawable != null) {
			mRightDrawable.setBounds(leftBounds, topBounds, rightBounds,
					bottomBounds);
		}
		if (mBottomDrawable != null) {
			mBottomDrawable.setBounds(leftBounds, topBounds, rightBounds,
					bottomBounds);
		}
		mTextView.setCompoundDrawables(mLeftDrawable, mTopDrawable,
				mRightDrawable, mBottomDrawable);
	}
	 ...
	 *            the bottom
	 */
	public void setTabCompoundDrawablesBounds(int left, int top, int right,
			int bottom) {
