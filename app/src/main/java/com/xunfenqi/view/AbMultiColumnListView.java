...
/**
 * 
 * @ClassName: AbMultiColumnListView
 * @Description:
 * @author Xuebo Li
 * @date 2015-8-14 下午2:55:40
 * 
 */
public class AbMultiColumnListView extends ScrollView {
	...
	public AbMultiColumnListView(Context context, AttributeSet attrs,
		...
		scrollLayout.addView(firstColumn, new LinearLayout.LayoutParams(0,
				LayoutParams.WRAP_CONTENT, 1));
		scrollLayout.addView(secondColumn, new LinearLayout.LayoutParams(0,
				LayoutParams.WRAP_CONTENT, 1));
		scrollLayout.addView(thirdColumn, new LinearLayout.LayoutParams(0,
				LayoutParams.WRAP_CONTENT, 1));
		this.addView(scrollLayout, new LinearLayout.LayoutParams(
			...
			if (!checkVisibility(i)) {
				if (viewInfo.getVisible() == View.VISIBLE) {
					...
					for (int id : mReleaseImageResIds) {
						imageView = (ImageView) viewInfo.getView()
								.findViewById(id);
	 ...
	 * 
	 * @param releaseImageResIds
	 *            the new release image res ids
	 */
