package com.xunfenqi.view.progress;
...
/**
 * 
 * @ClassName: AbHorizontalProgressBar
 * @Description:
 * @author Xuebo Li
 * @date 2015-9-7 下午3:52:01
 * 
 */
...
public class AbHorizontalProgressBar extends View {
	...
	private int[] fillColors = new int[] { 0xFF4FADCF, 0xFFFFFF00, 0xFFD75344,
			0xFFD75643 };
	...
	private double mAProgress = 50;

	public double getmAProgress() {
		return mAProgress;
	}

	public void setmAProgress(double mAProgress) {
	...
	protected void onDraw(Canvas canvas) {
		...
		f = new RectF(0, 0, this.width, this.height);
		//canvas.drawRect(0, 0, this.width, this.height, pathPaint);
		canvas.drawRoundRect(f, round, round, pathPaint);
		...
		// LinearGradient linearGradient = new LinearGradient(0, 0, this.width,
		// this.height,
		// fillColors[0], fillColors[1], TileMode.CLAMP);

		LinearGradient linearGradient = new LinearGradient(0, 0, this.width,
				this.height, fillColors, null, TileMode.CLAMP);
		...
		f = new RectF(0, 0, ((float) progress / max) * this.width, this.height);

		canvas.drawRoundRect(f, round, round, fillPaint);
