package com.xunfenqi.view.ninelock;
...
import com.xunfenqi.utils.TDevice;
...
/**
 * 
 * @ClassName: GestureDrawline
 * @Description: 手势密码路径绘制
 * @author Xuebo Li
 * @date 2015-8-13 下午1:55:34
 * 
 */
public class GestureDrawline extends View {
	...
	public GestureDrawline(Context context, List<GesturePoint> list,
		...
		super(context);
		screenDispaly = TDevice.getScreenDispaly(context);
		...
		paint.setColor(Color.parseColor("#5D8CF0"));// 设置默认连线颜色
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		paint.setColor(Color.rgb(93, 140, 240));// 设置默认连线颜色
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (currentPoint != null) {
				currentPoint.setPointState(AbConstant.POINT_STATE_SELECTED);
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (pointAt == null
					...
					|| AbConstant.POINT_STATE_SELECTED == pointAt
							.getPointState()) {
				...
				pointAt.setPointState(AbConstant.POINT_STATE_SELECTED);

				// 判断是否中间点需要选中
				GesturePoint betweenPoint = getBetweenCheckPoint(currentPoint,
						pointAt);
				if (betweenPoint != null
						&& AbConstant.POINT_STATE_SELECTED != betweenPoint
								.getPointState()) {
					// 存在中间点并且没有被选中
					...
					// 设置中间点选中
					betweenPoint.setPointState(AbConstant.POINT_STATE_SELECTED);
					// 赋值当前的point;
					currentPoint = pointAt;
				} else {
	...
	final class clearStateRunnable implements Runnable {
		public void run() {
			...
			for (GesturePoint p : list) {
				p.setPointState(AbConstant.POINT_STATE_NORMAL);
			}
			invalidate();
			isDrawEnable = true;
		}
	...
	private void drawErrorPathTip() {
		...
		for (Pair<GesturePoint, GesturePoint> pair : lineList) {
			pair.first.setPointState(AbConstant.POINT_STATE_WRONG);
			pair.second.setPointState(AbConstant.POINT_STATE_WRONG);
			canvas.drawLine(pair.first.getCenterX(), pair.first.getCenterY(),
					pair.second.getCenterX(), pair.second.getCenterY(), paint);// 画线
		}
		invalidate();
