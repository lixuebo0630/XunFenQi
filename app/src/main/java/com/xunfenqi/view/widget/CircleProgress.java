...
public class CircleProgress extends View {
	...
	public CircleProgress(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		default_text_size = UIUtils.dip2px(18);
		min_size = (int) UIUtils.dip2px(100);

		final TypedArray attributes = context.getTheme()
				.obtainStyledAttributes(attrs, R.styleable.CircleProgress2,
						defStyleAttr, 0);
	...
	protected void initByAttributes(TypedArray attributes) {
		finishedColor = attributes.getColor(
				R.styleable.CircleProgress2_circle_finished_color,
				default_finished_color);
		unfinishedColor = attributes.getColor(
				R.styleable.CircleProgress2_circle_unfinished_color,
				default_unfinished_color);
		textColor = attributes.getColor(
				R.styleable.CircleProgress2_circle_text_color,
				default_text_color);
		textSize = attributes
				.getDimension(R.styleable.CircleProgress2_circle_text_size,
						default_text_size);

		setMax(attributes.getInt(R.styleable.CircleProgress2_circle_max,
				default_max));
		setProgress(attributes.getInt(
				R.styleable.CircleProgress2_circle_progress, 0));

		if (attributes
				.getString(R.styleable.CircleProgress2_circle_prefix_text) != null) {
			setPrefixText(attributes
					.getString(R.styleable.CircleProgress2_circle_prefix_text));
		}
		if (attributes
				.getString(R.styleable.CircleProgress2_circle_suffix_text) != null) {
			setSuffixText(attributes
					.getString(R.styleable.CircleProgress2_circle_suffix_text));
		}
	}
